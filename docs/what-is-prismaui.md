...
PrismaUI takes a different approach: it brings **Chromium** (CEF 147 — the same browser engine inside VS Code, Electron, Spotify, and dozens of other apps) into Fallout 4 as a separate rendering layer. Your plugin's UI is just an HTML file that Chromium renders. You never touch Scaleform or ActionScript at all.
...
**C++ ↔ JS bridge.** Call JavaScript functions from your C++ plugin, and register C++ callbacks that your JavaScript can fire. Two directions, low overhead, works across the thread boundary.
...
**It does not require special build tools for your HTML.** A text editor is enough. Drop your `.html`, `.css`, and `.js` files in the right folder and they work. No bundler is required (though you can use one if you want).
